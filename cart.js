...
let cartItemsEl = document.querySelector(".popup-content");
...
function renderSubtotal() {
  ...
  subtotalEl.innerHTML = `Rs.${item.totalPrice} &nbsp; &nbsp; &#8250;`;
  cartCountEl.innerHTML = `${item.totalItems} items`;
}
...
function renderCartItems() {
  cart.forEach((item) => {
    ...
    cartItemsEl.innerHTML += `
            ...
            <div class="item-img" onclick = "removeItemFromCart('${item.id}')">
              <img src="${item.imageURL}" alt="${item.name}">
            </div>
            <div class="item-info">
              <h3>${item.name}</h3>
              <div class="item-count">
                <div class="decrement" onclick="updateCount('decrement', '${item.id}')">-</div>
                <div class="number"> ${item.numberOfUnits} </div>
                <div class="increment" onclick="updateCount('increment', '${item.id}')">+</div>
...
//function for update count of an item
function updateCount(action, id) {
  cart = cart.map((item) => {
    let numberOfUnits = item.numberOfUnits;
    if (item.id === id) {
      if (action === "decrement" && numberOfUnits > 1) {
        numberOfUnits--;
      } else if (action === "increment" && numberOfUnits < item.stock) {
        numberOfUnits++;
      }
    }
    return {
