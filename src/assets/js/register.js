...
var signin_form = document.querySelector('.signin-action-form');
    if(signin_form){
...
if(login_form){
    login_form.addEventListener('submit',function(e){
        ...
        var span = document.querySelector('#showerror'); 
            ...
            if(window.localStorage){
                var localStorageData =  localStorage.getItem('formdata');
                var checkEmail = JSON.parse(localStorageData) ? JSON.parse(localStorageData).filter((e)=>e.email == email && e.password == password) : [];
