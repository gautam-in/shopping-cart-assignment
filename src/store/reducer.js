...
function reducer(state = appState, action) {
  ...
  switch (type) {
    case UPDATE_CART_ITEM:
      if (state.cart.find((e) => e.id === _.get(payload, "id"))) {
        return {
          ...state,
          item: state.item + 1,
          cart: state.cart.map((ele) => {
            if (ele.id === _.get(payload, "id")) {
    ...
    case REMOVE_CART_ITEM:
      const findItem = state.cart.find(
        (element) => element.id === _.get(payload, "id")
      );
      if (findItem) {
        if (findItem.qty > 1) {
          return {
          ...
          return {
            ...state,
            item: state.item - 1,
            cart: state.cart.filter((ele) => ele.id !== _.get(payload, "id")),
          };
        }
      } else {
