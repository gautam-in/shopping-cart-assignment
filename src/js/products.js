...
export default class Products {
  ...
  addToCart = dataObj => {
    ...
    isLocalUpdated
      ? this.cart.handleCartView()
      : console.error('cart not updated')
    return true
  }
  ...
  handleCartData = event => {
    ...
    if (className === 'btn-cta' || className === 'btn-cta mobile tablet') {
      const { prodId, prodName, prodPrice, prodImg } = dataset
