...
function updateCartTotal() {
  ...
  if (sessionStorage.getItem("cart")) {
    ...
    const cartHeadingContainer = document.createElement("div");
    cartHeadingContainer.classList.add("sub-heading", "bg-color-white", "margin-bottom-sm", "padding-sm", "cart-heading");

    section.appendChild(cartHeadingContainer);

    const cartHeading = document.createElement("h3");
    cartHeading.innerText  = "My Cart ("+ items +" items)";

    cartHeadingContainer.appendChild(cartHeading);

    const cartDescription = document.createElement("div");
    cartDescription.classList.add("sub-heading", "bg-color-white", "margin-bottom-sm", "padding-sm", "cart-description-container");
    section.appendChild(cartDescription);

    //loop over cart array
    let totalPrice = 0;
    for (var i = 0; i < items; i++) {
      const desImg = document.createElement("div");
      cartDescription.appendChild(desImg);
      ...
      img.classList.add("img-size-md");
      img.src  = itemDes.imageURL;
      desImg.appendChild(img);

      const des = document.createElement("div");
      cartDescription.appendChild(des);
  
      const name  = document.createElement("h3");
      name.innerText  = itemDes.name;
  
      des.appendChild(name);

      const priceDes = document.createElement("div");
      cartDescription.appendChild(priceDes);
  
      const price  = document.createElement("p");
      price.classList.add("text-align");
      price.innerText  = "Rs. "+itemDes.price;
      totalPrice+=itemDes.price;
      
      priceDes.appendChild(price);
