...
const getCategories = async () => {
  const unorderedList = document.querySelector(".category-list");
  for (let i = 0; i < categories.length; i++) {
    ...
    itemLink.text = categories[i].name;
    itemLink.classList.add("main-nav-link");
    listItem.appendChild(itemLink);

    itemLink.addEventListener("click", () => {
      getProducts(categories[i].id);
    });
  }
};
...
const getProducts = async (catId) => {
  ...
  if (catId) {
    for (let j = 0; j < products.length; j++) {
      if (products[j].category == catId) productByCat.push(products[j]);
    }
  } else {
  ...
  let productsDiv = document.querySelector(".product-items-container");
  if (productsDiv.children.length > 0) {
    productsDiv.remove();
    productsDiv = document.createElement("div");
    productsDiv.classList.add("product-items-container");
    section.appendChild(productsDiv);
  }

  for (let i = 0; i < productByCat.length; i++) {
    const productItem = document.createElement("div");
    productItem.classList.add("product-item");
    productsDiv.appendChild(productItem);

    const productCard = document.createElement("div");
    productCard.classList.add("text-align");
    productItem.appendChild(productCard);

    const productName = document.createElement("h3");
    productName.classList.add("sub-heading");
    productName.innerText = productByCat[i].name;
    productCard.appendChild(productName);

    const productImg = document.createElement("img");
    productImg.classList.add("img-size-md");
    productImg.src = `${productByCat[i].imageURL}`;
    productImg.alt = productByCat[i].name;
    productCard.appendChild(productImg);

    const productDescription = document.createElement("p");
    productDescription.innerText = productByCat[i].description;
    productDescription.classList.add("content", "padding-sm", "bg-color-grey");
    ...
    productPrice.innerText = "MRP " + productByCat[i].price;
    productActionsRow.appendChild(productPrice);

    const productBuyDesktop = document.createElement("button");
    productBuyDesktop.addEventListener("click", function () {
      addToCart(products[i].id);
    });
    productBuyDesktop.classList.add("buy-now-btn");
    productBuyDesktop.innerText = "Buy Now";
    productBuyDesktop.value = productByCat[i].id;
