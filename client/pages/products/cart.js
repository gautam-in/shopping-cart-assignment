class Cart {
  ...
  render = () => {
    ...
    document
      .getElementById("checkout__btn")
      .addEventListener("click", this.removeCart);
    document
      .getElementById("close__cart")
      .addEventListener("click", this.removeCart);
  ...
  showCart = () => {
    if (this._itemDetail.length === 0) {
      document.getElementById(
        "cart__items__count"
      ).innerText = `My Cart (${this._itemDetail.length} item)`;
      document.getElementById("lowest-price").style.visibility = "hidden";
    } else {
  ...
  addCartItem = (item) => {
    if (this._itemDetail.filter((e) => e.id === item[0].id).length > 0) {
      ...
      document
        .getElementsByTagName("my-header")[0]
        ._shadowRoot.getElementById(
          "cart-count"
        ).innerText = `${this._itemDetail.length} Items`;
  ...
  updateCartItemCount = (item, action) => {
    if (
      !(
        (item.count === 0 && action === -1) ||
        (item.count === item.stock && action === 1)
      )
    ) {
      item.count += action;
      document.getElementById("count__" + item.id).textContent = item.count;
      document.getElementById("pc__productprice__" + item.id).textContent =
...
class CartItem {
  ...
  render() {
    document.getElementById("cart__container__main").insertAdjacentHTML(
      "afterbegin",
      `
        ...
        this._product.count
            ...
            <div class="pc__productprice" id="pc__productprice__${
              this._product.id
            }">Rs. ${this._product.price * this._product.count}</div>
        </div>
        `
    );

    document
      .getElementById("decreaseCount__" + this._product.id)
      .addEventListener("click", () => {
        this._cart.updateCartItemCount(this._product, -1);
      });

    document
      .getElementById("increaseCount__" + this._product.id)
      .addEventListener("click", () => {
        this._cart.updateCartItemCount(this._product, +1);
      });
  }
