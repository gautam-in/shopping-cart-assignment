...
function displayItems() {
  ...
  if (products.length) {
    ...
    document.getElementById("number_Products").innerHTML = `My Cart (0 items)`;
...
function itemAction(e) {
  if (e.target && e.target.nodeName == "BUTTON") {
    ...
    const cartTotal = parseInt(
      document.getElementById("cart_total").innerHTML.split(" ")[1]
    );
    ...
    if (action === "+") {
      ...
      document.getElementById("cart_total").innerHTML = `Rs. ${
        cartTotal + price
      } >`;
    }
    if (action === "-") {
      ...
      if (prev > 1) {
        ...
        document.getElementById(
          "number_Products"
        ).innerHTML = `My Cart (${cart.length} items)`;
        document.getElementById(
          "total_count"
