...
const Register = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if(isValidEmail(email) && isValidPassword(password) && password === cpassword && fName !== '' && lName !== '')
    {
        setEmailError(null);
    ...
    <>
      ...
      <section className={`${styles.registerSection} pageSection`}>
        ...
        <form className={styles.registerForm} onSubmit={handleSubmit}>
            <input type="text" placeholder="First Name" aria-label="First Name" value={fName} onChange={(e)=> {setFName(e.target.value)}}/>
            {fNameError && <span className={styles.errorMsg}>{fNameError}</span>}
            <input type="text" placeholder="Last Name" aria-label="Last Name" value={lName} onChange={(e)=> {setLName(e.target.value)}}/>
            {lNameError && <span className={styles.errorMsg}>{lNameError}</span>}
            <input type="text" placeholder="Email" value={email} aria-label="Email" onChange={(e)=> {setEmail(e.target.value)}}/>
            ...
            <input type="password" placeholder="Confirm Password" aria-label="Confirm Password" value={cpassword} onChange={(e)=> {setCPassword(e.target.value)}}/>
            {cPasswordError && <span className={styles.errorMsg}>{cPasswordError}</span>}
            <button type="submit" className={styles.formCta}>Sign up</button>
        </form>
      </section>
    </>
  );
