    ...
    return (
        <div className="mini-cart-container">
            <div className="cart-title-close-icon-container">
                <div>{`My Cart ${cartProducts.length!==0 ?cartProducts.length===1?'('+cartProducts.length+' item)':'('+cartProducts.length+' items)' :''}`}</div><div><IconButton style={{color:'#fff'}} onClick={handleCartCloseClick}><CloseIcon /></IconButton></div>
            </div>           
            {cartProducts.length ? cartProducts.map(cartProduct => {
                return (
                    <div className="product-carts-items" key={cartProduct.id}>
                        <div className="carted-image-box"><img className="carted-image" src={cartProduct.imageURL}/></div>
                        <div className="carted-item-content-box">
                            <div className="carted-product-name">{cartProduct.name}</div>
                            <div className="add-remove-item-content-info"><IconButton onClick={handleDecrementCartClick(cartProduct)}><RemoveCircleIcon className="remove-item-icon"/></IconButton><span className="carted-item-quantity spacing">{cartProduct.quantity}</span><IconButton onClick={handleIncrementCartClick(cartProduct)}><AddCircleIcon className="add-item-icon"/></IconButton><CloseOutlinedIcon className="multiply-icon spacing"/><span className="carted-product-price spacing">{cartProduct.price}</span></div>
                        </div>
            ...
            {cartProducts.length!==0 && (<div className="price-guarnteed-info-banner">
                <div><img className="price-guarnteed-info-image" src="/static/images/lowest-price.png" /></div>
                <div className="price-guarnteed-info-content">You won't find it cheaper anywhere</div>
            </div>)}
            <div className="mini-cart-footer">
                {cartProducts.length!==0&&<div>Promo code can be applied on payment page</div>}
                <div><Button  onClick={handleCartCloseClick} className="mini-cart-footer-button" variant="contained">{cartProducts.length?<React.Fragment><span>Proceed to Checkout</span><span>Rs.{'187'} {'>'}</span></React.Fragment>:<span>Start Shopping</span>} </Button></div>
            </div>
        </div>
    )
}