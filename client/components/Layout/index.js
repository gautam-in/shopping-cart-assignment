...
const Layout = ({ children }) => {
    ...
    useEffect(() => {
        let count = 0, cost = 0;
        Object.keys(products).forEach((p) => { count += products[p].count; cost = cost + (products[p].count * products[p].price) });
        setTotalCount(count)
        setTotalCost(cost)
    }, [products])
    ...
    return (
        <>
            <Header toggleCart={() => setShowCart(!showCart)} cartCount={totalCount} />
            <main>
                {showCart && <Cart
