export class Modal extends HTMLElement {
  ...
  connectedCallback() {
    ...
    document.querySelectorAll("#decrement").forEach((node) => {
      ...
      node.addEventListener("click", () => {
        if (parentEle.querySelector("#counter").innerText > 0) {
          ...
          parentEle.querySelector("#item-total").innerText =
            parseInt(parentEle.querySelector("#item-price").innerText) *
            parseInt(parentEle.querySelector("#counter").innerText);
        } else {
        }
      });
    });
    ...
    document.querySelectorAll("#increment").forEach((node) => {
      let parentEle = node.parentElement;
      node.addEventListener("click", () => {
        parentEle.querySelector("#counter").innerText =
          parseInt(parentEle.querySelector("#counter").innerText) + 1;
        parentEle.querySelector("#item-total").innerText =
          parseInt(parentEle.querySelector("#item-price").innerText) *
          parseInt(parentEle.querySelector("#counter").innerText);
      });
