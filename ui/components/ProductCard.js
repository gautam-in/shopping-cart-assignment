...
function ProductCard(props) {
    ...
    const handleAddCart=(pdt)=>{
        console.log(pdt)
        if(typeof(getCookie("cartItems"))=="string"){var carts=JSON.parse(getCookie("cartItems"));}
        else{var carts=cartItems};
        console.log(carts,f)
        var f=0;
    ...
    return (
        <div className="card"  >
            <CardStyle >
            <Card.Img variant="top" src={`${props.pdts.imageURL}`} className="cImage"/>
            <Card.Body className="cBody">
                <Card.Title>{props.pdts.name}</Card.Title>
                <Card.Text >
                {props.pdts.description}...
                </Card.Text>
            </Card.Body>
            </CardStyle>
            <Button variant="danger" onClick={e=>{return handleAddCart(props.pdts)}}>Buy Now @ Rs{props.pdts.price}</Button>
        </div>
    )
}
