...
// api category url
const baby_api_url = "http://localhost:5000/baby";
const baby_url_static = "../server/baby/index.get.json"
...
async function getapi() {
  ...
  let response;
  try {
    response = await fetch(baby_api_url);
  } catch {
    response = await fetch(baby_url_static);
  }
  // Storing baby_data in form of JSON
  var baby_data = await response.json();
  show(baby_data);
}
...
function show(baby_data) {
  let tab;
  // Loop to access all rows
  for (let r of baby_data) {
    ...
    document.getElementById("babydata").appendChild(divData);
  ...
  function openModal(product, name, image, price) {
    let babyData = [localStorage.getItem("product")];
    var cartData = babyData || [];
