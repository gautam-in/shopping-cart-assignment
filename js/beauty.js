...
// api category url
const beauty_api_url = "http://localhost:5000/beauty";

// Defining async function
async function getapi(beauty_url) {

  // Storing response
  const response = await fetch(beauty_url);
  // Storing beauty_data in form of JSON
  var beauty_data = await response.json();
  show(beauty_data);
}

// Calling that async function
getapi(beauty_api_url);

// Function to define innerHTML for HTML table
function show(beauty_data) {
  let tab;
  // Loop to access all rows
  for (let r of beauty_data) {
    ...
    document.getElementById("beautydata").appendChild(divData);
  ...
  function openModal(product, name, image, price) {
    let beautyData = [localStorage.getItem("product")];
      var cartData = beautyData || [];
