...
// api category url
const bakery_api_url = "http://localhost:5000/bakery";

// Defining async function
async function getapi(bakery_url) {

  // Storing response
  const response = await fetch(bakery_url);
  // Storing bakery_data in form of JSON
  var bakery_data = await response.json();
  show(bakery_data);
}

// Calling that async function
getapi(bakery_api_url);

// Function to define innerHTML for HTML table
function show(bakery_data) {
  let tab;
  // Loop to access all rows
  for (let r of bakery_data) {
    ...
    document.getElementById("bakerydata").appendChild(divData);
  ...
  function openModal(product, name, image, price) {
    let bakeryData = [localStorage.getItem("product")];
      var cartData = bakeryData || [];
