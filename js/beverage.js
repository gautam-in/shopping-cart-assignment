...
// api category url
const beverage_api_url = "http://localhost:5000/beverages";

// Defining async function
async function getapi(beverage_url) {

  // Storing response
  const response = await fetch(beverage_url);
  // Storing beverage_data in form of JSON
  var beverage_data = await response.json();
  show(beverage_data);
}

// Calling that async function
getapi(beverage_api_url);

// Function to define innerHTML for HTML table
function show(beverage_data) {
  let tab;
  // Loop to access all rows
  for (let r of beverage_data) {
    ...
    document.getElementById("beveragedata").appendChild(divData);
  ...
  function openModal(product, name, image, price) {
      ...
      if (productData == null) {
        productData =  [localStorage.setItem("product", [JSON.stringify(product)])];
     } else {
 
     var oldData = [JSON.parse(productData)] || [];
    oldData.push(product);
    localStorage.setItem("product", JSON.stringify(oldData));
    var cartCount = oldData.length;
     }
